...
class DeConv(Decoder):
    def __init__(self, in_channels, kernel_size,skip_type,additional_input_ch):
...
class UpConv(Decoder):
    ...
    def __init__(self, in_channels,skip_type,additional_input_ch):
        ...
        self.layer2 = self.upconv_module(in_channels // 2 + additional_input_ch[1])
        self.layer3 = self.upconv_module(in_channels // 4 + additional_input_ch[2])
        self.layer4 = self.upconv_module(in_channels // 8 + additional_input_ch[3])
...
class UpProj(Decoder):
    ...
    def __init__(self, in_channels,skip_type,additional_input_ch):
        ...
        self.layer1 = self.UpProjModule(in_channels + additional_input_ch[0],in_channels // 2)
        self.layer2 = self.UpProjModule(in_channels // 2 + additional_input_ch[1],in_channels // 4)
        self.layer3 = self.UpProjModule(in_channels // 4 + additional_input_ch[2],in_channels // 8)
        self.layer4 = self.UpProjModule(in_channels // 8 + additional_input_ch[3],in_channels // 16)
...
class ResNet(nn.Module):
    def __init__(self,
        ...
        extra_channels = (0,256,128,64) if skip_type == "cat" else (0,0,0,0)
        self.decoder = choose_decoder(decoder, num_channels // 2,extra_channels,skip_type)
